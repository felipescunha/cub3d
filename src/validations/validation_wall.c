...
/*   Updated: 2023/07/21 18:52:01 by fecunha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int    check_wall(t_cub3d *cub3d, size_t line, size_t col)
{
    ...
    {
        if (line != 0 && col != 0 && character(cub3d->full_map[line - 1][col - 1]))
            return (1);
        if (col != 0 && character(cub3d->full_map[line][col - 1]))
            return (1);
        if (line < cub3d->temp_line && col != 0
            && character(cub3d->full_map[line + 1][col - 1]))
            return (1);
        if (line != 0 && character(cub3d->full_map[line - 1][col]))
            return (1);
        if (character(cub3d->full_map[line][col]))
            return (1);
        if (line < cub3d->temp_line && character(cub3d->full_map[line + 1][col]))
            return (1);
        if (line != 0 && col < cub3d->temp_collunm
            && character(cub3d->full_map[line - 1][col + 1]))
            return (1);
        if (col < cub3d->temp_collunm && character(cub3d->full_map[line][col + 1]))
            return (1);
